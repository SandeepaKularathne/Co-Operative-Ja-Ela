...
export class GrnComponent {
  ...
  initialize() {
    ...
    this.gss.getAllList().then((vsts: Grnstatus[]) => {
      this.grnstatuses = vsts;
    });

    this.es.getAll('').then((vtys: Employee[]) => {
      this.employees = vtys;
    });

    this.ps.getAll('').then((vbrs: Purorder[]) => {
      this.purorders = vbrs;
    });
  ...
  createForm() {
    ...
    this.form.controls['grnstatuse'].setValidators([Validators.required]);
  ...
  add() {
    ...
    if (errors != "") {
      ...
      confirm.afterClosed().subscribe(async result => {
        if (result) {
          // @ts-ignore
          this.is.add(this.grn).then((responce: [] | undefined) => {
